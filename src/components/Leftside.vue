...
import AdvancedMessage from './AdvancedMessage.vue'; 
...
<template>
        ...
        <div class="bg-main rounded-xl px-3 py-2 flex justify-between">
            <div class="flex gap-4 items-center"> 
                ...
                <p class="text-transparent bg-text-gradient font-bold bg-clip-text text-sm">DM Tour Guide</p>
            </div>
            ...
            <div
                ...
                <circle-progress :percent="appStore.guideProgress" :is-gradient="true" :border-width="20" :linecap="0" :gradient="{
                    ...
                    startColor: '#40BEBE',
                    stopColor: '#2361E6'
                }" empty-color="rgba(23, 82, 101, 0.05)" class="!absolute inset-0 !w-7 !h-7 scale-x-flip" />
                <p class="text-gray-900/50 text-mv font-bold"><span class="text-sky text-xs">{{ appStore.guideStepsCompleted || 0 }}</span>/2</p>
            </div>
        </div>

        <!-- Manage Inbox -->
        <div class="space-y-4">
            <h4 class="font-bold text-gray-400 uppercase text-mv tracking-widest px-4">Manage Inbox</h4>
            <Menu />
        </div>
        <!-- Advanced Messages -->
        <div class="space-y-4 ">
